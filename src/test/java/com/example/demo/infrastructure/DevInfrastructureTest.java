...
public class DevInfrastructureTest {
    ...
    public void testLoadBalancerHealth() {
        try (ElasticLoadBalancingV2Client elbClient = ElasticLoadBalancingV2Client.builder()
            ...
            LoadBalancer devLoadBalancer = response.loadBalancers().stream()
                    .filter(lb -> lb.loadBalancerName().equals(ALB_NAME))
                    .findFirst()
                    .orElseThrow(() -> new RuntimeException("DEV Load Balancer not found"));
            
            assertEquals("active", devLoadBalancer.state().code().toString(),
